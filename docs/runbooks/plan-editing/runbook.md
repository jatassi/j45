...
3. **`bun run test`** — exit 0; 48 files / 200 tests, including:
   ...
   - `packages/client/test/workout-draft.test.ts` uniform expand/collapse + decode failures + `27 works · 27:10`
...
4. **`bun run test:e2e`** — exit 0; 35 passed, 1 skipped (passkey webkit).
   ...
   - chromium+webkit: Athletica copy edit (summary `27 works · 27:10`, laps→sets, rename, station-down, Save, detail+reload, empty station name disables Save + visible error)

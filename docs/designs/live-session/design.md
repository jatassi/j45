...
> takes the edit at once, and the frozen milliseconds are re-derived from the
> segment now in force, so the resume counts down the segment it is actually
> in. A done session is frozen: no later change reaches it, and its completion
> row keeps the plan that was in force while the timer ran. A new plan that no
> longer reaches the current work ordinal finishes the session on the plan it
> was running — the alternative, a clamp backwards, replays a station the
> participant completed. That finish raises no notice and moves no revision,
> so a participant cannot tell it from the last rep of the plan.
>

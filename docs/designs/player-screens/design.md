...
directly on a phase-tinted backdrop inside a depleting progress arc, with a
...
- **Center stack**: phase eyebrow with hue dot; the countdown — the single
  ...
  arm's length from the floor — wrapped in a thin SVG **progress arc** that
...
- **Demo chip**: a small inline media chip below the arc — the reserved
...
- **States**: paused → digits keep last interpolated value, arc frozen,
...
- **Idle**: work / rest / rounds as kit `field` inputs (stepper affordances,
  ...
  audio in-tap.
- **Running**: identical center stack (phase tint, arc, digits, round
...
ends), the done-state Finish-as-leave, and arc/phase-attribute assertions

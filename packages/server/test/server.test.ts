...
describe('server', () => {
  ...
  it.scopedLive(
    'a request for a directory inside the build falls back to the entry document',
    () =>
      Effect.gen(function* () {
        const distDir = yield* makeDistDir
        const port = yield* bootServer({ releaseSha: 'test-sha-dir', clientDistDir: distDir })

        // `assets/` is present but is not a file anything can stream — it
        // takes the client-side-routing fallback rather than failing.
        const directory = yield* get(port, '/assets')
        expect(directory.status).toBe(200)
        expect(yield* bodyOf(directory)).toBe(entryHtml)
      }),
    { timeout: 20_000 },
  )

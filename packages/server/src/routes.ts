...
import { ClientDistDir } from "./clientDist.js"
    ...
    Effect.gen(function* () {
      ...
      yield* router.get(
        ...
        Effect.gen(function* () {
          ...
          const isInsideDist =
            requestedPath === absoluteDistDir ||
            requestedPath.startsWith(absoluteDistDir + path_.sep)

          if (isInsideDist) {
            const requestedExists = yield* fs.exists(requestedPath).pipe(

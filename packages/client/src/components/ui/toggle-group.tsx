...
/**
 * Deviates from the CLI output by one class. Base UI's Toggle emits
 * `data-pressed` / `aria-pressed`, never `data-state="on"`, so the CLI's
 * Radix-flavoured `data-[state=on]:bg-muted` matched nothing. The on-state
 * fill comes from `aria-pressed:bg-muted` in `toggleVariants` above. Do not
 * restore it on a CLI re-add.
 */
